...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	AverageSpeed := meanSpeed(action, duration) * kmhInMsec //СредняяСкоростьВМетрахВСекунду
	x := math.Pow(AverageSpeed, 2) / (height / cmInM)
	// ваш код здесь
	// ((0.035 * ВесСпортсменаВКг + (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах) * 0.029 * ВесСпортсменаВКг) * ВремяТренировкиВЧасах * minInH)
	return (walkingCaloriesWeightMultiplier*weight + x*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	AverageSpeed := swimmingMeanSpeed(lengthPool, countPool, duration) //СредняяСкоростьВКм/ч
